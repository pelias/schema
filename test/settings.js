    ...
    for (var analyzerName in s.analysis.analyzer) {
      ...
      if (Array.isArray(analyzer.char_filter)) {
        analyzer.char_filter.forEach(function (charFilterName) {
          ...
          if (!filterExists && ES_INBUILT_FILTERS.indexOf(charFilterName) > -1) {
            filterExists = true
          }
...
module.exports.tests.punctuationCharFilter = function (test, common) {
  test('has punctuation char_filter', function (t) {
    ...
    var charFilter = s.analysis.char_filter.punctuation
    t.equal(charFilter.type, 'mapping')
    t.true(Array.isArray(charFilter.mappings))
    t.equal(charFilter.mappings.length, 48)
    t.end()
  })
}
...
module.exports.tests.alphanumericCharFilter = function (test, common) {
  test('has alphanumeric char_filter', function (t) {
    ...
    var charFilter = s.analysis.char_filter.alphanumeric
    t.equal(charFilter.type, 'pattern_replace')
    t.equal(charFilter.pattern, '[^a-zA-Z0-9]')
    t.equal(charFilter.replacement, '')
    t.end()
  })
}
...
module.exports.tests.numericCharFilter = function (test, common) {
  test('has numeric char_filter', function (t) {
    ...
    var charFilter = s.analysis.char_filter.numeric
    t.equal(charFilter.type, 'pattern_replace')
    t.equal(charFilter.pattern, '[^0-9]')
    t.equal(charFilter.replacement, ' ')
    t.end()
  })
}
...
module.exports.tests.overrides = function (test, common) {
  test('override defaults', function (t) {
    process.env['PELIAS_CONFIG'] = path.resolve(path.join(__dirname, '/fixtures/empty.json'))

    var s = settings()
    t.equal(s.index['number_of_replicas'], '0', 'unchanged')

    // set the PELIAS_CONFIG env var
    process.env['PELIAS_CONFIG'] = path.resolve(path.join(__dirname, '/fixtures/config.json'))
  ...
  test('override similarity', function (t) {
    ...
    // set the PELIAS_CONFIG env var
    process.env['PELIAS_CONFIG'] = path.resolve(path.join(__dirname, '/fixtures/similarity.json'))
